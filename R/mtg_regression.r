...
This script loads draft game data provided to the public by 17lands. 
It converts the draft data into a matrix suitable for machine learning algorithms.
Each column of the Decks matrix corresponds to a unique card in the Streets of New Capenna set
Each row corresponds to a unique deck entry found in the 17lands data
Each value represents the number of copies of a given card played in the main deck.
Basic lands are omitted.

Each row i of the Decks matrix (D) maps to element i of the rates vector
The rates vector stores the observed win rate of the given deck i.e. Player One built deck 3 and we observed win rate 3 (6-3) = .667
